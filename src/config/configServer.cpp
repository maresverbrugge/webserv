...
static void get_upload_folder_from_config(server_t& server_info, std::vector<std::string> words)
{
	if (words.size() < 2)
		config_error_message("Invalid number of arguments for upload_directory directive.");
	else
		server_info.upload_folder = words[1];
}
...
static void create_full_server_paths(server_t& server_info)
{
	...
	if (server_info.upload_folder.length() > 0)
		server_info.upload_folder = server_info.root_folder + server_info.upload_folder;
}
